...
import {
  counselingTypeObject1,
  relationshipStatusObject2,
  therapyBeforeObject3,
  SpecificQualitiesObject4,
  issuesObject5,
} from './BookingFormsObject';
...
function BookingForms({ step, setStep, formData, setFormData }) {
  ...
  function FormStyle() {
    ...
    if (step < 6 || step > 6) {
      formstyle =
        'p-6 mt-9 mb-12 mx-auto w-auto md:w-[700px] h-auto sm:h-[500px] border-2 border-[#E5E5E5] rounded-md shadow-lg bg-white';
    }
    return formstyle;
  }
    ...
    if (step <= 6) {
      button = (
        <button
          type="submit"
          disabled={
            (step === 1 && formData.counseling_type === null) ||
            (step === 2 && formData.relationship_status === null) ||
            (step === 3 && formData.ever_been_in_therapy_before === null) ||
            (step === 4 && formData.counselor_qualities === null) ||
            (step === 5 && formData.issues === null) ||
            (step === 6 &&
              (formData.what_brings_you_here === null ||
                !formData.what_brings_you_here.trim().length))
          }
          onClick={() => {
  ...
  function DisplayRadioButtons() {
    ...
    if (step === 1) {
      radioButtons = counselingTypeObject1.map(({ id, value, label }) => {
        return (
          <div
            className="form-check pt-4 text-2xl sm:text-3xl font-normal"
            key={id}
          >
            <input
              type="radio"
              ...
              onChange={() => {
                setChecked(value);
                setFormData({ ...formData, counseling_type: value });
              }}
            />
          ...
          >
            <input
              ...
              onChange={() => {
                ...
                setFormData({
                  ...formData,
                  ever_been_in_therapy_before: value,
                });
              }}
            />
            <label className="form-check-label" htmlFor={id}>
        ...
        return (
          <div key={id} className="flex py-3">
            <input
              ...
              onChange={() => {
                setChecked(value);
                setFormData({ ...formData, counselor_qualities: value });
              }}
            />
      ...
      radioButtons = (
        <input
          ...
          name="paragraph"
          onChange={(e) =>
            setFormData({ ...formData, what_brings_you_here: e.target.value })
          }
  ...
  return (
    <div>
      <form onSubmit={handleSubmit} className={FormStyle()}>
        <h1
          className={`text-2xl sm:text-3xl font-normal pb-8 ${
            step > 6 ? 'text-center sm:pt-16 sm:pb-10' : 'text-left'
          }`}
        >
          {ChangeFormQuestion()}
        </h1>
        ...
        {step > 6 && (
          <p className="text-center text-xl sm:text-2xl px-2 sm:px-16 sm:py-10">
            {step === 7
